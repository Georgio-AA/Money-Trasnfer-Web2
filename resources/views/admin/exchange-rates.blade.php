...
<div class="admin-container">
    <div class="page-header">
        ...
        <form action="{{ route('admin.exchange-rates.sync') }}" method="POST" style="display: inline;">
            @csrf
                        ...
                        <tr>
                            <td>
                                <div class="currency-pair">
                                    <strong>{{ $rate['from'] }}</strong>
                                    <span class="arrow">→</span>
                                    <strong>{{ $rate['to'] }}</strong>
                                </div>
                            </td>
                            <td><span class="rate-value">{{ number_format($rate['rate'], 6) }}</span></td>
                            <td><span class="updated-time">{{ \Carbon\Carbon::parse($rate['updated_at'])->diffForHumans() }}</span></td>
                            <td>
                                <form action="{{ route('admin.exchange-rates.delete-rate', $key) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete this exchange rate?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="fees-tab" class="tab-content">
        <div class="card">
            ...
            <form action="{{ route('admin.exchange-rates.update-fee') }}" method="POST">
                @csrf
                <div class="form-grid">
                    <div class="form-group">
        ...
        <div class="card">
            ...
            <table class="fees-table">
                ...
                <tbody>
                    @forelse($fees as $currency => $fee)
                        <tr>
                            <td><strong>{{ $fee['currency'] }}</strong></td>
                            <td><span class="badge badge-{{ $fee['fee_type'] }}">{{ ucfirst($fee['fee_type']) }}</span></td>
                            <td>
                                @if($fee['fee_type'] === 'percentage')
                                    {{ $fee['fee_percentage'] }}%
                                @elseif($fee['fee_type'] === 'fixed')
                                    {{ number_format($fee['fee_fixed'], 2) }} {{ $fee['currency'] }}
                                @else
                                    Tiered Structure
                                @endif
                            </td>
                            <td>{{ number_format($fee['min_fee'], 2) }} {{ $fee['currency'] }}</td>
                            <td>{{ $fee['max_fee'] ? number_format($fee['max_fee'], 2) . ' ' . $fee['currency'] : 'No limit' }}</td>
                            <td><span class="updated-time">{{ \Carbon\Carbon::parse($fee['updated_at'])->diffForHumans() }}</span></td>
                            <td>
                                <form action="{{ route('admin.exchange-rates.delete-fee', $currency) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete this fee structure?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="action-btn delete">Delete</button>
                                </form>
