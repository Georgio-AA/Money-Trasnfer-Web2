...
<div class="fraud-container">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="subtext">{{ $stats['alerts_today'] }} today</div>
        </div>
        <div class="stat-card orange">
            <h3>High Risk Transfers</h3>
            <div class="value">{{ $stats['high_risk_transfers'] }}</div>
            <div class="subtext">Requiring review</div>
        </div>
        <div class="stat-card green">
            <h3>Blocked Users</h3>
            <div class="value">{{ $stats['blocked_users'] }}</div>
            <div class="subtext">{{ $stats['blocked_ips'] }} IPs blocked</div>
        </div>
        <div class="stat-card blue">
            <h3>Fraud Prevented</h3>
            <div class="value">${{ number_format($stats['fraudulent_amount'], 2) }}</div>
            <div class="subtext">{{ $stats['prevented_transactions'] }} transactions (7 days)</div>
        </div>
    </div>
                ...
                <table class="alerts-table">
                    ...
                    <tbody>
                        @foreach($alerts as $alert)
                            <tr>
                                <td><code>{{ substr($alert['id'], 0, 12) }}</code></td>
                                <td>
                                    <strong>{{ $alert['user_name'] }}</strong><br>
                                    <small style="color: #6b7280;">{{ $alert['user_email'] }}</small>
                                </td>
                                <td><a href="{{ route('admin.transfers.show', $alert['transfer_id']) }}" style="color: #3b82f6;">#{{ $alert['transfer_id'] }}</a></td>
                                <td><strong>${{ number_format($alert['amount'], 2) }}</strong></td>
                                <td>
                                    <span class="fraud-score score-{{ $alert['severity'] }}">
                                        {{ $alert['fraud_score'] }}%
                                    </span>
                                </td>
                                <td>
                                    <span class="risk-badge risk-{{ $alert['severity'] }}">
                                        {{ strtoupper($alert['severity']) }}
                                    </span>
                                </td>
                                <td>
                                    <span class="status-badge status-{{ $alert['status'] === 'pending' ? 'pending' : ($alert['status'] === 'confirmed_fraud' ? 'confirmed' : 'false') }}">
                                        {{ str_replace('_', ' ', ucfirst($alert['status'])) }}
                                    </span>
                                </td>
                                <td><small>{{ \Carbon\Carbon::parse($alert['created_at'])->diffForHumans() }}</small></td>
                                <td>
                                    @if($alert['status'] === 'pending')
                                        <div class="action-btns">
                                            <button class="btn btn-sm btn-success" onclick="reviewAlert('{{ $alert['id'] }}', '{{ $alert['transfer_id'] }}')">Review</button>
                                            <button class="btn btn-sm btn-primary" onclick="viewReasons('{{ json_encode($alert['reasons']) }}')">Details</button>
                                        </div>
                                    @else
                                        <span style="color: #9ca3af; font-size: 13px;">
                                            Reviewed by {{ $alert['reviewed_by'] ?? 'Unknown' }}
                                        </span>
                                    @endif
                                </td>
                            </tr>
                    ...
                    <tbody>
                        @foreach($rules as $rule)
                            <tr>
                                ...
                                <td><code style="background: #f3f4f6; padding: 4px 8px; border-radius: 4px;">{{ $rule['condition'] }}</code></td>
                                <td><span class="fraud-score score-{{ $rule['score_points'] >= 30 ? 'critical' : ($rule['score_points'] >= 20 ? 'high' : 'medium') }}">+{{ $rule['score_points'] }}</span></td>
                                <td>
                                ...
                                <td>
                                    <form method="POST" action="{{ route('admin.fraud.delete-rule', $rule['id']) }}" style="display: inline;" onsubmit="return confirm('Delete this rule?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    <tbody>
                        @foreach($blocked['users'] as $item)
                            <tr>
                                ...
                                <td><small>{{ \Carbon\Carbon::parse($item['blocked_at'])->format('M d, Y H:i') }}</small></td>
                                <td>
                                    <form method="POST" action="{{ route('admin.fraud.unblock') }}" style="display: inline;" onsubmit="return confirm('Are you sure you want to unblock this user?');">
                                        @csrf
                    ...
                    <tbody>
                        @foreach($blocked['ips'] as $item)
                            <tr>
                                ...
                                <td><small>{{ \Carbon\Carbon::parse($item['blocked_at'])->format('M d, Y H:i') }}</small></td>
                                <td>
                                    <form method="POST" action="{{ route('admin.fraud.unblock') }}" style="display: inline;">
                                        @csrf
        ...
        <div class="section-card">
            ...
            @if(count($highRiskTransfers) > 0)
                <table class="alerts-table">
                    ...
                    <tbody>
                        @foreach($highRiskTransfers as $transfer)
                            <tr>
                                <td><strong>#{{ $transfer->id }}</strong></td>
                                <td>{{ $transfer->user->name ?? 'Unknown' }}</td>
                                ...
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('admin.transfers.show', $transfer->id) }}" class="btn btn-sm btn-primary">View Details</a>
                                        <button class="btn btn-sm btn-warning" onclick="viewReasons('{{ $transfer->fraud_reasons }}')">Reasons</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
...
<div id="addRuleModal" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" action="{{ route('admin.fraud.add-rule') }}">
            @csrf
            <div class="modal-body">
                <div class="form-group">
